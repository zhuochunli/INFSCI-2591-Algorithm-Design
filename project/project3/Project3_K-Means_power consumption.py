...
if __name__ == '__main__':
    data = pd.read_csv('E:/学习资料/Pitt/algorithm/project/project3/Project3_Power_Consumption.csv')
    dataset = []
    for i in range(len(data)):
        dataset.append([float(data['Global_active_power'].values[i]), float(data['Global_reactive_power'].values[i]),
                        float(data['Voltage'].values[i]), float(data['Global_intensity'].values[i]),
                        float(data['Sub_metering_1'].values[i]), float(data['Sub_metering_2'].values[i]), float(data['Sub_metering_3'].values[i])])
    for k in range(2, 21):
        print('For K={}:'.format(k))
        centroid, belong_to, loss = kmeans(dataset, k, 50)
